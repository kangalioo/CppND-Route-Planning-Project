...
std::tuple<std::vector<RouteModel::Node>, float> construct_final_path(RouteModel::Node &end_node) {
    auto distance = 0.0f;
    // We hold pointers here instead of references because C++ is a hackfest and only support
    // vectors of references with an ugly wrapper
...
std::tuple<std::vector<RouteModel::Node>, float> plan_the_route(RouteModel &model, float start_x, float start_y, float end_x, float end_y) {
    ...
    auto [path_found, distance] = construct_final_path(end_node);
    distance *= model.MetricScale(); // Multiply the distance by the scale of the map to get meters.
    return {path_found, distance};
}
